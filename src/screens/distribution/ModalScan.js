...
const ModalScan = ({
  ...
  const fetchBox_API = async (item_no) => {
    ...
    setBox(box?.data.filter((el) => el.is_scan !== 'SCANED'))

    console.log(box?.data.filter((el) => el.is_scan !== 'SCANED'))
  }
  ...
  useEffect(() => {
    if (box?.length > 0) {
      box?.length === box?.filter((el) => el.is_scan_d === 'SCANED').length
        ? setCheckStatus(true)
  ...
  const rows = Array.from({length: numInputs}, (_, index) => {
    ...
    const isScanned = matchingBox?.is_scan_d === 'SCANED'
